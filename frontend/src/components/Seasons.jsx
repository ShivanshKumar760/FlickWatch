...
export default function Seasons({id,season_number}) {
  ...
  // Fetch season data when component is mounted
  useEffect(() => {
    async function fetchSeason() {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_BACKEND_API}/api/v1/tv/${id}/season/${season_number}`
        );
        const fetchedSeasonData = await response.json();
        // console.log("this is season",fetchedSeasonData);
        setSeason(fetchedSeasonData.content);
      } catch (error) {
    ...
    fetchSeason();
  }, [id, season_number]); // Added id and season as dependencies
  ...
  return (
    <>
      <div className="relative space-y-2 lg:px-10 p-6 bg-black">
        <h1 className="font-semibold tracking-tighter text-xl lg:text-3xl text-white">
          {season && season.name}
        </h1>
